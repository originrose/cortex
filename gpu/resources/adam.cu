...
void adam_step_d( double alpha, double beta1, double beta2, double epsilon, double pow_beta1_t, double pow_beta2_t, double gradient_beta,
		  ...
		  int parameterCount )
{
  adam_step(alpha, beta1, beta2, epsilon, pow_beta1_t, pow_beta2_t, gradient_beta, gradients, parameters, m, v, parameterCount);
}

extern "C"
__global__
void adam_step_f( double alpha, double beta1, double beta2, double epsilon, double pow_beta1_t, double pow_beta2_t, double gradient_beta,
		  float* gradients, float* parameters, float* m, float* v,
		  int parameterCount )
{
  adam_step(static_cast<float>(alpha), static_cast<float>(beta1),
	    static_cast<float>(beta2), static_cast<float>(epsilon),
	    static_cast<float>(pow_beta1_t), static_cast<float>(pow_beta2_t),
	    static_cast<float>(gradient_beta), gradients, parameters, m, v, parameterCount);
}
