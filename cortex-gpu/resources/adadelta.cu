...
extern "C"
__global__
void adadelta_step_f( double decay, double epsilon,
		      float* grad_accum_ary, float* dx_accum_ary,
		      double gradient_beta,
		      float* gradient_ary, float* parameters_ary,
		      int count )
{
  adadelta_step( static_cast<float>(decay), static_cast<float>(epsilon), grad_accum_ary, dx_accum_ary,
		 static_cast<float>(gradient_beta), gradient_ary, parameters_ary,
		 count );
}
